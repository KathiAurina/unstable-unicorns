...
export function isCardBasicDueToEffect(playerEffects: PlayerEffectEntry[], card: { type: CardType | CardType[] }): boolean {
    if (!hasEffect(playerEffects, "my_unicorns_are_basic")) return false;
    if (hasEffect(playerEffects, "pandamonium")) return false;
    return hasType(card, "narwhal") || hasType(card, "unicorn");
}